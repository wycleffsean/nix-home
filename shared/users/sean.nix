{
  ...
  staticKeys = [
    ...
    ''
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAINEcrvoi6zR128a98G1lprVLYUPF+yUokY5Yl0MYro5S id_nix"
  ];
  ...
  addIfMissing = key: acc:
    if builtins.elem key acc
    then acc
    else acc ++ [key];
  combineUnique = list: let
    recur = xs: acc:
      if xs == []
      then acc
      else recur (builtins.tail xs) (addIfMissing (builtins.head xs) acc);
  in
    recur list [];
